...
use App\Models\Area;
use App\Models\Reportedre;

class ReportedreController extends Controller
{
    ...
    {   $dependencia = Auth::user()->dependencia_id;
        ...
        { 
                if ($dependencia == 1)
                {
                    $reportedres = Reportedre::all()->where('l_activo', 'S');
                   
                }
                else 
                {
                    $reportedres = Reportedre::all()->where('dependencia_id', $dependencia);
                    //$colegios = Colegio::all()->where('l_activo', 'S')->where('dependencia_id', $dependencia);
                }
       
         return view('sistema.reportes.reportedres')->with('reportedres',$reportedres);
        } else {
