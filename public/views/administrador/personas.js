new Vue({
    ...
    data: {
        ...
        search: {
            'filter': 1,
            'text_filter': 'Buscar por personas',
            'datos': null
        },
        send: '',
        page: 1,
    ...
    methods: {
        ...
        Buscar(page) {
            ...
            urlBuscar = 'personas/buscar?page=' + page;
            axios.post(urlBuscar, {
                filter: this.search.filter,
                search: this.search.datos,
            }).then(response => {
                this.Load('my_table', 'off', null);
                this.listRequest = response.data.personas.data;
                this.to_pagination = response.data.personas.to;
        ...
        Modal(size, metodo, id, seleccion) {
            ...
            switch (metodo) {                
                ...
                case 'edit':
                    this.persona.nombre = seleccion.x_nombre;
                    this.persona.dni = seleccion.c_dni;
                    this.persona.telefono = seleccion.x_telefono;
                    this.persona.email = seleccion.x_email;
