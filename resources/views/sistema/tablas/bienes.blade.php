...
@section('content')
            ...
            <div class="row">
                ...
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        ...
                        <li class="breadcrumb-item active">Bienes</li>
                    </ol>
                </div>
            </div>
        </div>
    ...
    <section class="content">
        <div class="container-fluid">
            <div class="row" style="padding-top: 10px;" id="bienes">
                <div class="col-12">
                    <div class="card card-danger card-outline">
                        <div class="card-header">
                            <h3 class="card-title">LISTA DE BIENES</h3>
                            ...
                            <div class="modal-dialog" role="document" :class="modal.size">
                                        ...
                                        <div class="m-2">
                                            <div class="form-row">
                                                ...
                                                <div class="form-group col-md-12">
                                                    <label for="grupo">GRUPO GENÉRICO <span class="obligatorio">*</span></label>
                                                    <select id="grupo" v-model="bien.grupo" class="form-control" :class="[errors.grupo ? 'border-error' : '']">
                                                        <option value="">--- Seleccione Opción ---</option>
                                                        @foreach ($grupos as $grupo)
                                                            <option value="{{$grupo->id}}">{{$grupo->c_codigo.': '.$grupo->x_nombre}}</option>
                                                        @endforeach
                                                    </select>
                                                    <div class="input-error" v-if="errors.grupo">@{{ errors.grupo[0] }}</div>
                                                </div>
                                                <div class="form-group col-md-12">
                                                    <label for="clase">CLASE GENÉRICO <span class="obligatorio">*</span></label>
                                                    <select id="clase" v-model="bien.clase" class="form-control" :class="[errors.clase ? 'border-error' : '']">
                                                        <option value="">--- Seleccione Opción ---</option>
                                                        @foreach ($clases as $clase)
                                                            <option value="{{$clase->id}}">{{$clase->c_codigo.': '.$clase->x_nombre}}</option>
                                                        @endforeach
                                                    </select>
                                                    <div class="input-error" v-if="errors.clase">@{{ errors.clase[0] }}</div>
                                                </div>
                                                <div class="form-group col-md-12">
                                                    <label for="codigo">CÓDIGO <span class="obligatorio">*</span></label>
                                                    <input type="text" id="codigo" v-model="bien.codigo" class="form-control" :class="[errors.codigo ? 'border-error' : '']" maxlength="8" onkeypress="Numero()" autocomplete="off">
                                                    <div class="input-error" v-if="errors.codigo">@{{ errors.codigo[0] }}</div>
                                                </div>
                                                <div class="form-group col-md-12">
                                                    <label for="nombre">NOMBRE DEL BIEN <span class="obligatorio">*</span></label>
                                                    <input type="text" id="nombre" v-model="bien.nombre" class="form-control" :class="[errors.nombre ? 'border-error' : '']">
                                                    <div class="input-error" v-if="errors.nombre">@{{ errors.nombre[0] }}</div>
                                                </div>
                                                <div class="form-group col-md-12">
                                                    <label for="resolucion">RESOLUCIÓN </label>
                                                    <input type="text" id="resolucion" v-model="bien.resolucion" class="form-control" :class="[errors.resolucion ? 'border-error' : '']" maxlength="100">
                                ...
                                <div class="modal-content" v-if="modal.method == 'delete'" id="delete">
                                    <div class="modal-body text-center" style="padding: 0.8rem 1.3rem">
                                        ...
                                        <div class="m-2">
                                            <p class="text-center">¿ Realmente desea eliminar el Bien: <br>
                                                <strong>@{{bien.nombre}}</strong> ?</p>
                                ...
                                <table class="table table-md table-hover">
                                    <thead>
                                        <tr style="background-color: #f2f2f2;">
                                            ...
                                            <th width="45%">NOMBRE DEL BIEN</th>
                                            <th width="15%" class="text-center">GRUPO</th>
                                            <th width="15%" class="text-center">CLASE</th>
                                    ...
                                    <tbody>
                                        <tr v-for="(data, index) in listRequest" :class="'active-'+data.l_activo">
                                            ...
                                            <td>@{{data.x_nombre}}</td>
                                            <td class="text-center">@{{data.get_grupo.x_nombre}}</td>
                                            <td class="text-center">@{{data.get_clase.x_nombre}}</td>
...
@section('js')
    <script src="{{asset('views/tablas/bienes.js')}}"></script>
@endsection